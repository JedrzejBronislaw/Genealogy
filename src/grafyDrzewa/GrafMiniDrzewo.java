...
public class GrafMiniDrzewo extends GrafDrzewa {
	...
	public GrafMiniDrzewo() {}
	public GrafMiniDrzewo(Osoba osoba) {
		this.osobaGlowna = osoba;
	}
	...
	public void rysuj(Graphics2D g) {
		...
		Osoba[] rodzenstwo = osobaGlowna.getRodzenstwo();
...
//		g.drawLine(szerokosc-marginesX, wysokosc-marginesY, marginesX, wysokosc-marginesY);
	...
	{
		Osoba matka = osobaGlowna.getMatka();
		Osoba ojciec = osobaGlowna.getOjciec();
		Osoba[] rodzenstwo = osobaGlowna.getRodzenstwo();
		String nazwisko = osobaGlowna.imieNazwisko();
	...
	private void rysujRodzenstwo(Graphics2D g, Osoba[] rodzenstwo, int x, int y)
	{
		Font f = g.getFont();
	...
	private int szerRodzenstwa(Graphics2D g, Osoba[] rodzenstwo) {
		Font f = g.getFont();
		g.setFont(new Font(f.getName(), Font.PLAIN, f.getSize()));
