...
public class GrafStdPotomkowie extends GrafDrzewa{
...
//	}
	...
	public GrafStdPotomkowie() {}
	public GrafStdPotomkowie(/*Drzewo d,*/ Osoba o) {
//		drzewo = d;
		osobaGlowna = o;
	}
...
//		g.drawLine(0, marginesY+wysokosc, szerokosc+marginesX*2, marginesY+wysokosc);
	...
	private void szerokosciGalezi(FontMetrics fm, Osoba osoba, int pokolenie)
	{
		int szerokosc;
		Osoba malzonek;
	...
	private int rysujRodzine(Graphics2D g, Osoba o, int x, int y, int pokolenie)
	{
		int przes = 0;
		int przesMal = 0;
...
//			przes -= miedzyRodzenstwem; //Mo¿na w³¹czyæ do zacieœnienia zapisu (i do mniejszenia czytlnoœci)
	...
	private void rysujNumerMalzenstwa(Graphics2D g, Osoba dziecko, Osoba rodzic, int x1, int y1, int yLini) {
	...
	private int rysujRodzine(Graphics2D g, Osoba o, int x, int y, int pokolenie, boolean ostatnie)
	{
		int przes = 0;
		int przesMal = 0;
//		FontMetrics fm = g.getFontMetrics();
...
//			przesMal += rysujMalzonka(g, o.getMalzonek(i), x, y+((odstepMiedzyMalzonkami+wysNazw)*(i+1))) + odstepMiedzyMalzonkami;
	...
	private int rysujMalzonka(Graphics2D g, Osoba o, int x, int y)
	{
//		String nazwisko = o.imieNazwisko();
//		FontMetrics fm = g.getFontMetrics();
